const urlParser = require("./parser.js");
...
function runSearch(e) {
  if (e.keyCode == 13) {
    ...
    let insertArea = document.querySelector(".ul");
...
// then
async function processData(searchInput) {
  console.log(searchInput);
  if (searchInput === undefined) {
    searchInput = "india";
  }
  let data = await urlParser(searchInput);
  console.log(data);
  let insertArea = document.querySelector(".ul");
  ...
  data.map((station) => {
    insertArea.insertAdjacentHTML(
      ...
      `
      <li
        data-url=${station.stationIP}
        data-title="${station.stationTitle}"
        data-location="${searchInput}"
        class="station"
      >
...
function clickThis(event) {
  ...
  if (t.classList.contains("station")) {
    ...
    document.querySelector(".location").innerHTML = t.getAttribute(
      "data-location"
    );
  }
}
...
function indicatorShow(el) {
  el.innerHTML =
    '<div class="icon-sound"><span></span><span></span><span></span><span></span></div>';
}
...
function volumeSet(event) {
  let left = document
    .querySelector(".progress-volume-outer")
  ...
  document
    .querySelector(".icon-volume")
    .classList.remove("volume-mute", "volume-low", "volume-high");
  if (volume > 0)
    document.querySelector(".icon-volume").classList.add("volume-mute");
  if (volume >= 33)
    document.querySelector(".icon-volume").classList.add("volume-low");
  if (volume >= 66)
    document.querySelector(".icon-volume").classList.add("volume-high");
